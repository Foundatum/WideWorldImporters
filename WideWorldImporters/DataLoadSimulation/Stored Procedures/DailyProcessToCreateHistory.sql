...
CREATE PROCEDURE [DataLoadSimulation].[DailyProcessToCreateHistory]
    @StartDate                           date
  , @EndDate                             date
  , @AverageNumberOfCustomerOrdersPerDay int = 15
  ...
  , @MinSeasonalVariationPercent         int = -5
  , @MaxSeasonalVariationPercent         int = 20
	...
	BEGIN
		...
		FROM (SELECT COUNT(*) AS OrderCount FROM Sales.Orders 
			WHERE DATEPART(year,OrderDate) = DATEPART(year,(SELECT MAX(OrderDate) FROM Sales.Orders))
				...
				AND BackorderOrderID IS NULL --These need to be exluded so that if you run the process
											 --multiple times that the backordered invoices are not accounted
											 --for in the estimate.
	...
	BEGIN TRY
		...
		-- compute number of orders to process
			...
			SET @DateMessage = N'Processing '
							 ...
							 + N' Days Remaining; '
							 + CAST(@NumberOfCustomerOrders AS NVARCHAR) 
							 + N' Customer Orders '
							 ;
